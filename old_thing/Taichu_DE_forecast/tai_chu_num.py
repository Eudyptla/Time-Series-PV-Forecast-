...
trainset = np.arange(1,11)/10
for i in trainset:
    trainset_index = int(len(x)*i)
    testX = x.iloc[:trainset_index,:]
    y = testX.iloc[:, 0]
    X = testX.iloc[:, 1:]

    train_idx = int(len(testX) * .8)
...
# #
# #
    error = mean_squared_error(y_test, y_rbf1)
    # print('Test MSE: %.3f' % error)
    Root_error = error ** 0.5
    print('Test RMSE: %.3f' % Root_error)
    # R_2 = r2_score(y_test, y_rbf1)
    # print('Test R^2: %.3f' % R_2)
    # Ab_error = mean_absolute_error(y_test, y_rbf1)
    # print('Test MAE: %.3f' % Ab_error)

    draw_pl = draw_pl+[trainset_index,Root_error]
