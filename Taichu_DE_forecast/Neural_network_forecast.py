...
model.add(Dense(1, W_regularizer=l2(reg), activity_regularizer=L1L2(reg)))
...
plt.title("SOLAR POWER PREDICTION - RECURRRENT NEURAL NETWORKS")
plt.legend([line11, line12])
plt.show()
print('ACCURACY PREDICTION OF VALUE:', 1 - np.mean(abs(bb)))
print('ACCURACY SOLAR POWER PREDICTION:', float(sum(x > 0 for x in bb) / len(bb)), '(worse than random guess)')
print('')
print('DIFFERENCE SIGNAL REAL - PREDICTED')

error = mean_squared_error(testY, testPredict)
